...
### [v0.4.7.0] - Dashboard Remaster & System Statistics
> **Time:** 2025-11-30 19:55
*   **🏠 Dashboard Reconstruction (首页重构):**
    *   **Homepage Overhaul:** 将首页从资产监控改造为工业控制终端（Industrial Operating System），采用模块化卡片布局。
    *   **Module Cards:** 新增三大模块入口卡片（协议档案库、音频终端、蓝图模块），支持 Hover 发光效果与点击跳转。
    *   **System Status Bar:** 实时显示系统状态、文件数、音乐数等统计信息，带脉冲呼吸动画。
    *   **Activity Log:** 新增终端风格的实时活动日志组件，每小时自动刷新，展示最近文件上传记录。
*   **📐 Editor Module Placeholder (编辑器模块占位):**
    *   **Construction Page:** 新增 `/editor` 占位页面，显示"功能建设中"提示与旋转齿轮动画。
    *   **Future Ready:** 预留蓝图编辑器入口，为未来 Three.js 3D 编辑功能做准备。
*   **📊 Backend Statistics API (后端统计接口):**
    *   **Stats Endpoint:** 新增 `/stats/` 接口，提供文件数、音频数、用户数等系统统计数据。
    *   **Activity Endpoint:** 新增 `/activities/` 接口，返回最近5条文件上传活动记录。
*   **🧩 Component Development (组件开发):**
    *   **SystemStatusBar:** 状态栏组件，支持 TypeScript 类型定义，自动拉取统计数据。
    *   **ModuleCard:** 可复用的模块卡片组件，支持禁用状态、状态徽章、元信息展示。
    *   **ActivityLog:** 活动日志组件，终端绿色主题，支持定时自动刷新。


### [v0.4.6.0] - Music Player & Lyrics Protocol
> **Time:** 2025-11-27 17:55
*   **🎵 Music Player Terminal (音乐播放终端):**
    *   **Player Interface:** 上线完整的播放器页面 (`/play`)，采用左右分栏布局（封面展示区 + 歌词控制区）。
    *   **Lyrics System:** 实装 LRC 歌词解析器，支持实时同步歌词高亮、点击跳转、双语翻译切换。
    *   **Playback Modes:** 播放器支持 **顺序播放 (Sequence)** / **单曲循环 (Loop)** / **随机播放 (Shuffle)** 三种模式。
    *   **Cloud Sync:** 页面加载时自动拉取云端音频库，并启动定时刷新机制（50分钟周期）应对 R2 临时URL过期问题。
*   **🎤 Music Upload Module (音乐录入模块):**
    *   **Upload Page:** 新增音乐上传页面，支持 **音频母带**、**专辑封面**、**LRC歌词** 三合一上传。
    *   **Image Preview:** 封面上传支持实时预览，优化用户体验。
    *   **Data Binding:** 上传文件自动绑定艺术家信息与元数据，写入数据库后即时同步至播放列表。
*   **🐛 Critical Fixes (关键修复):**
    *   **Lyrics UI Bug:** 修复翻译按钮随歌词滚动移动的问题。
    *   **Playback Interrupt:** 修复点击播放列表按钮触发后端拉取导致播放中断的问题。
*   **🔧 Backend Enhancement (后端增强):**
    *   **Schema Update:** 数据库 `files` 表新增 `artist`（艺术家）、`cover_r2_key`（封面）、`lyrics_r2_key`（歌词）字段。
    *   **API Extension:** 文件接口支持返回音频元数据，前端可直接消费完整的播放列表数据。
*   **💄 UI/UX Polish (交互优化):**
    *   **Sidebar Player:** 优化侧边栏播放器，支持点击歌曲名跳转至播放器页面。
    *   **Wiki Integration:** 文件列表项组件支持直接播放音频文件，实现 Wiki 与播放器的无缝联动。
    *   **Design System:** 新增播放器专属样式。

### [v0.4.5.1] - Personnel Center & Safety Protocol
> **Time:** 2025-11-25 21:32
*   **🪪 Personnel Terminal (个人终端重构):**
    *   **Personal Dossier:** 新增档案组件。
    *   **Smart Editing:** 重构 ProfileCard。
    *   **Routing:** 重构 Settings 路由结构。
*   **🛡️ Security Protocol (安全协议):**
    *   **Safety Page:** 上线终端安全设置页面。
    *   **Core Functions:** 实装 **密码重置 (Password Reset)** 与 **通讯频段重定向 (Email Re-bind)** 功能，对接 Supabase Auth API。
    *   **Data Sync:** 数据库新增触发器，实现 `auth.users` 与业务表邮箱数据的实时同步。
*   **👮 Admin Command (管理指挥体系):**
    *   **Audit Center:** 独立 **干员审批中心**页面，实装分页查询。
    *   **Visual Differentiation:** 针对 **Admin**与 **Operator**设计了差异化的系统状态面板。
    *   **Logic Update:** 审批通过后，干员将自动归入 "基建工程部" 并保留所有档案数据。
